...
object FunSets {
   ...
   */
    def filter(s: Set, p: Int => Boolean): Set = x => if(s(x) && p(x)) true else false
   ...
   */
    def map(s: Set, f: Int => Int): Set = {
      var set1 = filter(s, x => false)
      for (i <- -bound to bound) {
        if(contains(s, i)) set1 = union(set1, singletonSet(f(i)))
      }
      set1
    }
    //x => if(s(x)) true else false
    //x => if(s((f(x)))) true else false
    //def map(s: Set, f: Int => Int): Set = x => if(s((f(x)))) true else false
